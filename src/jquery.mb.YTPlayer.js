  ...
  jQuery.mbYTPlayer = {
        ...
        $YTPlayer.one("YTAPIReady_" + YTPlayer.id, function () {
          ...
          jQuery(YTPlayer).on("YTPChanged", function (e) {
            ...
            $YTPlayer.on("YTPlayerIsReady_" + YTPlayer.id, function () {
               ...
               */
              jQuery(window)
                .off("orientationchange.YTP_" + YTPlayer.id)
                .on("orientationchange.YTP_" + YTPlayer.id, function () {
                  $YTPlayer.optimizeDisplay();
                });
    ...
    getDataFromAPI: function (YTPlayer) {
      ...
      if (YTPlayer.videoData) {
        ...
        jQuery
          .getJSON("https://www.googleapis.com/youtube/v3/videos?id=" + YTPlayer.videoID + "&key=" + jQuery.mbYTPlayer.apiKey + "&part=snippet", function (data) {
            YTPlayer.dataReceived = true;
    ...
    pause: function () {
      ...
      if (YTPlayer.opt.abundance < 0.2) this.YTPSetAbundance(0.2);
    ...
    buildControls: function (YTPlayer) {
       ...
       *  play/pause button
       * */
      let playpause = jQuery("<span>" + jQuery.mbYTPlayer.controls.play + "</span>")
       ...
       *  mute/unmute button
       * */
      let MuteUnmute = jQuery("<span>" + jQuery.mbYTPlayer.controls.mute + "</span>")
    ...
    checkForState: function () {
      ...
      YTPlayer.getState = setInterval(function () {
        ...
        if (
          ...
          (parseFloat(YTPlayer.player.getDuration() - YTPlayer.opt.fadeOnStartTime / 1000) < YTPlayer.player.getCurrentTime() || (stopAt > 0 && parseFloat(YTPlayer.player.getCurrentTime()) >= stopAt))
        ) {
...
})(jQuery, ytp); /*___________________________________________________________________________________________________________________________________________________
    ...
    return this.each(function () {
      ...
      if (e.CSSAIsRunning && e.eventType == k.type && !jQuery.browser.msie && 9 >= jQuery.browser.version)
        ...
        if (jQuery.support.CSStransition) {
          ...
          for (f in d)
            "transform" === f && delete d[f],
              "filter" === f && delete d[f],
              "transform-origin" === f && delete d[f],
              "auto" === d[f] && delete d[f],
              "x" === f && ((k = d[f]), (l = "left"), (d[l] = k), delete d[f]),
              "y" === f && ((k = d[f]), (l = "top"), (d[l] = k), delete d[f]),
              ("-ms-transform" !== f && "-ms-filter" !== f) || delete d[f];
  ...
  $.simpleSlider = {
    ...
    init: function (opt) {
      return this.each(function () {
        ...
        if (el.opt.maxval == "auto") el.opt.maxval = $(el).outerWidth();

        $el.updateSliderVal();

        if (!el.opt.readonly) {
          $el.on($.simpleSlider.events.start, function (e) {
            if ($.browser.mobile) e = e.changedTouches[0];

            el.canSlide = true;
            $el.updateSliderVal(e);

            if (el.opt.orientation == "h") $el.css({ cursor: "col-resize" });
            else $el.css({ cursor: "row-resize" });
          ...
          $(document)
            .on($.simpleSlider.events.move, function (e) {
              if ($.browser.mobile) e = e.changedTouches[0];
    ...
    updateSliderVal: function (e) {
      ...
      if (typeof e != "undefined") el.value = el.opt.orientation == "h" ? (el.x >= $el.outerWidth() ? el.opt.maxval : el.x <= 0 ? 0 : el.scaleX) : el.y >= $el.outerHeight() ? el.opt.maxval : el.y <= 0 ? 0 : el.scaleY;
      else el.value = el.opt.orientation == "h" ? el.scaleX : el.scaleY;

      if (el.opt.orientation == "h") el.level.width(getPercent(el.x, elWidth) + "%");
      else {
      ...
      if (el.lastVal === el.value && ((el.opt.orientation === "h" && (el.x >= $el.outerWidth() || el.x <= 0)) || (el.opt.orientation !== "h" && (el.y >= $el.outerHeight() || el.y <= 0)))) return;

      if (typeof el.opt.callback === "function") el.opt.callback(el);
...
})(jQuery); /*___________________________________________________________________________________________________________________________________________________
 _ jquery.mb.components                                                                                                                             _
 _                                                                                                                                                  _
